...
library LibArtifactUtils {
    ...
    function activateNonSpaceshipArtifact(
        ...
        if (artifact.artifactType == ArtifactType.Wormhole) {
            ...
            bool flag = false;
            if(artifact.rarity == ArtifactRarity.Unknown) flag = false;
            else if(artifact.rarity == ArtifactRarity.Common) flag = planet.planetLevel>=0 && planet.planetLevel<=3;
            else if(artifact.rarity == ArtifactRarity.Rare) flag = planet.planetLevel>=0 && planet.planetLevel<= 5;
            else if(artifact.rarity == ArtifactRarity.Epic) flag = planet.planetLevel>=0 && planet.planetLevel<= 7;
            else if(artifact.rarity == ArtifactRarity.Legendary) flag = planet.planetLevel>=0 && planet.planetLevel<= 8;
            else flag = true;
             require(
                // 3 * uint256(artifact.rarity) >= planet.planetLevel,
                flag,
            ...
            require(
                gs().planets[wormholeTo].owner != msg.sender && gs().planets[wormholeTo].owner!=address(0),
                "evil wormhole can only to a planet other own"
            );

            require(!gs().planets[wormholeTo].destroyed, "planet destroyed");

            require(gs().planets[wormholeTo].planetLevel <= planet.planetLevel,"wormholeTo planet level need equal or small");

              //add special score for activate evil wormhole
            gs().players[msg.sender].score += 3 * gameConstants().CAPTURE_ZONE_PLANET_LEVEL_SCORE[
                gs().planets[wormholeTo].planetLevel
            ];

            planet.destroyed = true;
            gs().planets[wormholeTo].destroyed = true;
    ...
    function deactivateArtifact(uint256 locationId) public {
        ...
        if (artifact.artifactType == ArtifactType.Wormhole) {
            gs().players[msg.sender].score -= 3 * gameConstants().CAPTURE_ZONE_PLANET_LEVEL_SCORE[
                gs().planets[artifact.wormholeTo].planetLevel
            ];

            planet.destroyed = false;
            gs().planets[artifact.wormholeTo].destroyed = false;
