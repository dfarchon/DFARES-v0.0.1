...
class GameManager extends EventEmitter {
      ...
      .on(ContractsAPIEvent.TxConfirmed, async (tx: Transaction) => {
        ...
        if (isUnconfirmedRevealTx(tx)) {
          ...
          await Promise.all([
            gameManager.hardRefreshPlanet(tx.intent.locationId),
            gameManager.hardRefreshArtifact(tx.intent.artifactId),
            gameManager.hardRefreshWormholeTo(tx.intent.wormholeTo),
          ]);
  ...
  public async hardRefreshWormholeTo(planetId: LocationId | undefined): Promise<void> {
    // this.terminal.current?.println(`wormhole to ${planetId}`, TerminalTextStyle.Green);
  ...
  public async hardRefreshArtifact(artifactId: ArtifactId): Promise<void> {
    ...
    await this.hardRefreshWormholeTo(oldArtifact?.wormholeTo);

    // this.terminal.current?.println(`${oldArtifact?.wormholeTo}`, TerminalTextStyle.Blue);

    const artifact = await this.contractsAPI.getArtifactById(artifactId);

    // this.terminal.current?.println(`${artifact?.wormholeTo}`, TerminalTextStyle.Red);

    await this.hardRefreshWormholeTo(artifact?.wormholeTo);
  ...
  public async deactivateArtifact(
    ...
    try {
      ...
      this.terminal.current?.println(`wormholeTo ${wormholeTo}`, TerminalTextStyle.Mythic);
